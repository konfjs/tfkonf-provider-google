...
export interface DataGoogleComputeRegionInstanceGroupManagerArgs {
  name?: string;
  project?: string;
  region?: string;
  self_link?: string;
...
export class data_google_compute_region_instance_group_manager extends TerraformResource {
  constructor(protected readonly terraformConfig: TerraformConfig, protected readonly resourceName: string, readonly args: DataGoogleComputeRegionInstanceGroupManagerArgs) {
    const meta = {};
    super(terraformConfig, "data", args, meta, "google_compute_region_instance_group_manager", resourceName);
  }
  ...
  get distribution_policy_target_shape(): string {
    return `data.${this.resourceType}.${this.resourceName}.distribution_policy_target_shape`;
  }

  get distribution_policy_zones(): string {
    return `data.${this.resourceType}.${this.resourceName}.distribution_policy_zones`;
  }
