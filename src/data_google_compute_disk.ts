...
export interface DataGoogleComputeDiskArgs {
  name: string;
  project?: string;
  zone?: string;
}

export class data_google_compute_disk extends TerraformResource {
  constructor(protected readonly terraformConfig: TerraformConfig, protected readonly resourceName: string, readonly args: DataGoogleComputeDiskArgs) {
    const meta = {};
    super(terraformConfig, "data", args, meta, "google_compute_disk", resourceName);
  }

  get access_mode(): string {
    return `data.${this.resourceType}.${this.resourceName}.access_mode`;
  }
  ...
  get enable_confidential_compute(): string {
    return `data.${this.resourceType}.${this.resourceName}.enable_confidential_compute`;
  }
  ...
  get provisioned_iops(): string {
    return `data.${this.resourceType}.${this.resourceName}.provisioned_iops`;
  }

  get provisioned_throughput(): string {
    return `data.${this.resourceType}.${this.resourceName}.provisioned_throughput`;
  }
  ...
  get source_image_encryption_key(): string {
    return `data.${this.resourceType}.${this.resourceName}.source_image_encryption_key`;
  }
  ...
  get storage_pool(): string {
    return `data.${this.resourceType}.${this.resourceName}.storage_pool`;
  }
